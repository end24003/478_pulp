...
# --- Default scenarios ---
DEFAULT_SCENARIOS = {
    'scenario1': {'name': 'Baseline', 'heat_treatment': 160, 'machining': 200, 'assembly': 180,
                  'demand_a': 50, 'demand_b': 80, 'profit_a': 90, 'profit_b': 60},
    'scenario2': {'name': 'Elevate Bottleneck', 'heat_treatment': 200, 'machining': 200, 'assembly': 180,
                  'demand_a': 50, 'demand_b': 80, 'profit_a': 90, 'profit_b': 60},
    'scenario3': {'name': 'Premium Product A', 'heat_treatment': 160, 'machining': 200, 'assembly': 180,
...
st.session_state.scenarios[selected]['name'] = st.sidebar.text_input(
    "Name:", value=st.session_state.scenarios[selected]['name'], key=f"n_{selected}"
)
st.session_state.scenarios[selected]['heat_treatment'] = st.sidebar.slider(
    "Heat Treatment:", 80, 240, st.session_state.scenarios[selected]['heat_treatment'], 10, key=f"h_{selected}"
)
st.session_state.scenarios[selected]['machining'] = st.sidebar.slider(
    "Machining:", 100, 300, st.session_state.scenarios[selected]['machining'], 10, key=f"m_{selected}"
)
st.session_state.scenarios[selected]['assembly'] = st.sidebar.slider(
    "Assembly:", 100, 300, st.session_state.scenarios[selected]['assembly'], 10, key=f"a_{selected}"
)
st.session_state.scenarios[selected]['demand_a'] = st.sidebar.slider(
    "Demand A:", 0, 100, st.session_state.scenarios[selected]['demand_a'], 5, key=f"da_{selected}"
)
st.session_state.scenarios[selected]['demand_b'] = st.sidebar.slider(
    "Demand B:", 0, 150, st.session_state.scenarios[selected]['demand_b'], 5, key=f"db_{selected}"
)
st.session_state.scenarios[selected]['profit_a'] = st.sidebar.slider(
    "Profit A ($):", 50, 150, st.session_state.scenarios[selected]['profit_a'], 5, key=f"pa_{selected}"
)
st.session_state.scenarios[selected]['profit_b'] = st.sidebar.slider(
    "Profit B ($):", 30, 100, st.session_state.scenarios[selected]['profit_b'], 5, key=f"pb_{selected}"
)
...
with col1:
    ...
    fig.add_trace(go.Bar(x=names, y=throughputs, marker_color=colors,
                         text=[f'${t:,.0f}' for t in throughputs], textposition='outside'))
